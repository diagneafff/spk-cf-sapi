<title>Identifikasi Diagnosa - Cattle 2.0</title>
<?php
switch ($_GET['act']) {
  ...
  default:
    if ($_POST['submit']) {
      ...
      while ($rpenyakit = mysqli_fetch_array($sqlpenyakit)) {
        $cftotal_temp = 0;
        $cf = 0;
        $riwayat = $rpenyakit['kode_penyakit'];
        $sqlgejala = mysqli_query($conn, "SELECT * FROM basis_pengetahuan where kode_penyakit = $riwayat");
        $cflama = 0;
        ...
        if ($cflama > 0) {
          $arpenyakit += array($riwayat => number_format($cflama, 4));
        }
      }
	...
	<h2 class='text text-primary'>Hasil Identifikasi Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil identifikasi diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      echo "</table><div class='well well-small'><img class='card-img-top img-bordered-sm' style='float:right; margin-left:15px;' src='" . $gambar . "' height=200><h3>Hasil Identifikasi Diagnosa</h3>";
      ...
      echo "
	 <h2 class='text text-primary'>Identifikasi Diagnosa Penyakit</h2>  <hr>
	 <div class='alert alert-success alert-dismissible'>
                <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
                <h4><i class='icon fa fa-exclamation-triangle'></i>Perhatian !</h4>
                Silahkan memilih gejala sesuai dengan kondisi sapi anda, anda dapat memilih kepastian kondisi sapi dari pasti tidak sampai pasti ya, jika sudah tekan tombol proses (<i class='fa fa-search-plus'></i>)  di bawah untuk melihat hasil.
              </div>
...
<?php
      ...
      echo "
		  <input class='float' type=submit data-toggle='tooltip' data-placement='top' title='Klik disini untuk melihat hasil identifikasi diagnosa' name=submit value='&#xf00e;' style='font-family:Arial, FontAwesome'>
          </tbody></table></form>";
    }
    break;
}
